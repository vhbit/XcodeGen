...
class SourceGenerator {
    ...
    private func getGroupSources(targetSource: TargetSource, path: Path, isBaseGroup: Bool) throws -> (sourceFiles: [SourceFile], groups: [(reference: String, group: PBXGroup)]) {
        ...
        if let baseLocalisedDirectory = baseLocalisedDirectory {
            for filePath in try baseLocalisedDirectory.children().sorted() {
                ...
                baseLocalisationVariantGroups.append(variantGroup.variantGroup)
                let buildFile = PBXBuildFile(fileRef: variantGroup.reference, settings: nil)
                let buildFileReference = generateReference(buildFile, variantGroup.reference + targetName)
                allSourceFiles.append(SourceFile(reference: buildFileReference, path: filePath, fileReference: variantGroup.reference, buildFile: buildFile, buildPhase: .resources))
            }
        }
            ...
            for filePath in try localisedDirectory.children().sorted { $0.lastComponent < $1.lastComponent } {
                ...
                if let variantGroup = variantGroup {
                    ...
                    let buildFile = PBXBuildFile(fileRef: fileReference,
                                                 settings: nil)
                    let buildFileReference = generateReference(buildFile, fileReference + targetName)
                    allSourceFiles.append(SourceFile(reference: buildFileReference, path: filePath, fileReference: fileReference, buildFile: buildFile, buildPhase: .resources))
